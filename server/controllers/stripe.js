...
export const createConnectAccount = async (req, res) => {
  ...
  // 3. create login link based on acount id (for frontend to complete onboarding)
...
export const payoutSetting = async (req, res) => {
  try {
    ...
    console.log("STRIPE PAYOUTSETTING ERROR", err);
  }
};
...
export const stripeSessionId = async (req, res) => {
  ...
  //2.find the hotel based on hoteId from db
  const item = await Hotel.findById(hotelId).populate("postedBy").exec();

  //3.charges on the plateform
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    //5.purchasing item details, it will be shown to user on the checkout
    line_items: [
      {
        name: item.title,
  ...
  //8.send session id as respone to frontend
  res.send({
...
export const stripeSuccess = async (req, res) => {
  try {
    ...
    //3.retrieve stripe session, based on sesion id we previously save in user db
